...
public final class ResourceGeneratorHelper {
  ...
  public static Identifier getItemModelId(@NotNull ItemConvertible item) {
    return (item instanceof ItemResourceGenerator) ? ((ItemResourceGenerator) item).getItemModelId() : Preconditions.checkNotNull(ForgeRegistries.ITEMS.getKey(item.asItem()), "The item is not registered.").brrp_prepend("item/");
  }
  ...
  public static Identifier getBlockModelId(@NotNull Block block) {
    return (block instanceof BlockResourceGenerator) ? ((BlockResourceGenerator) block).getBlockModelId() : Preconditions.checkNotNull(ForgeRegistries.BLOCKS.getKey(block), "The block is not registered.").brrp_prepend("block/");
  }
