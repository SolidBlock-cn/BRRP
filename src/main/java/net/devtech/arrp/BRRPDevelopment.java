...
public class BRRPDevelopment {
  ...
  /**
   * Create and register block items for the block. 为方块创建并注册对应的物品。
   */
  ...
  private static RuntimeResourcePack refreshPack(boolean clientIncluded, boolean serverIncluded) {
    ...
    if (serverIncluded) {
      ...
      ((IdentifiedTag) new IdentifiedTag(BlockTags.FENCES).addBlock(LAVA_FENCE)).write(PACK);
      ...
      ((IdentifiedTag) new IdentifiedTag(BlockTags.FENCE_GATES).addBlock(LAVA_FENCE_GATE)).write(PACK);
      ((IdentifiedTag) new IdentifiedTag(BlockTags.WALLS).addBlock(LAVA_WALL)).write(PACK);
      ((IdentifiedTag) new IdentifiedTag(ItemTags.FENCES).addItem(LAVA_FENCE)).write(PACK);
      ((IdentifiedTag) new IdentifiedTag(ItemTags.FENCE_GATES).addItem(LAVA_FENCE_GATE)).write(PACK);
      ((IdentifiedTag) new IdentifiedTag(ItemTags.WALLS).addItem(LAVA_WALL)).write(PACK);
      ((IdentifiedTag) new IdentifiedTag(BlockTags.PICKAXE_MINEABLE).addBlock(SMOOTH_STONE)).write(PACK);
  ...
  public static void registerPacks() {
    RRPEventHelper.BEFORE_VANILLA.registerPack(resourceType -> switch (resourceType) {
      case CLIENT_RESOURCES -> refreshPack(true, false);
      case SERVER_DATA -> refreshPack(false, true);
    });
