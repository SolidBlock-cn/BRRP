...
public class RuntimeResourcePackImpl extends AbstractRuntimeResourcePack implements ResourcePack {
  ...
  @Override
  public <T> byte[] addTag(TagKey<T> tagKey, Tag.Builder tagBuilder) {
    return addData(new Identifier(tagKey.id().getNamespace(), TagManagerLoader.getPath(tagKey.registry()) + "/" + tagKey.id().getPath() + ".json"), RuntimeResourcePack.serialize(tagBuilder.toJson()));
  }
