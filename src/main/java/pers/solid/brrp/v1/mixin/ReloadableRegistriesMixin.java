...
public abstract class ReloadableRegistriesMixin {
  ...
  private static <T> void brrp$prepareImmediate(LootDataType<T> type, MutableRegistry<?> mutableRegistry, Identifier identifier, Function<RegistryWrapper.WrapperLookup, ?> wrapperLookupFunction, ReloadableWrapperLookup wrapperLookup) {
    RuntimeResourcePackImpl.LOGGER.info("Adding immediate resource {} to registry {}", identifier, mutableRegistry.getKey().getValue());
    ((MutableRegistry<T>) mutableRegistry).add(RegistryKey.of(type.registryKey(), identifier), (T) wrapperLookupFunction.apply(wrapperLookup), DEFAULT_REGISTRY_ENTRY_INFO);
  }
}
