...
public class BRRPTest {
  ...
  private static void refreshPack(boolean clientIncluded, boolean serverIncluded) {
    ...
    if (serverIncluded) {
      ...
      PACK.addRecipeAndAdvancement(new Identifier("brrp", "smithing_transform"), SmithingRecipeJsonBuilder.create(Ingredient.ofItems(Items.DIAMOND), Ingredient.ofItems(Items.STONE), RecipeCategory.BUILDING_BLOCKS, Items.BEDROCK).setCustomRecipeCategory("brrp_custom").setBypassesValidation(true));
    }
