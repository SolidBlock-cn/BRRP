...
public interface BlockResourceGenerator extends ItemResourceGenerator {
  ...
  default Identifier getLootTableId() {
    if (this instanceof AbstractBlock) {
      final AbstractBlock block = (AbstractBlock) this;
  ...
  default void writeRecipes(RuntimeResourcePack pack) {
    ...
    if (shouldWriteStonecuttingRecipe()) {
      final SingleItemRecipeJsonFactory stonecuttingRecipe = getStonecuttingRecipe();
      if (stonecuttingRecipe != null) {
        pack.addRecipeAndAdvancement(getStonecuttingRecipeId(), stonecuttingRecipe::offerTo);
      }
    }
  }
