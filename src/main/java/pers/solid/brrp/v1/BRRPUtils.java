...
public final class BRRPUtils {
  ...
  public static Identifier getItemModelId(@NotNull ItemConvertible item) {
    return (item instanceof ItemResourceGenerator) ? ((ItemResourceGenerator) item).getItemModelId() : Registry.ITEM.getId(item.asItem()).brrp_prefixed("item/");
  }
  ...
  public static Identifier getBlockModelId(@NotNull Block block) {
    return (block instanceof BlockResourceGenerator) ? ((BlockResourceGenerator) block).getBlockModelId() : Registry.BLOCK.getId(block).brrp_prefixed("block/");
  }
