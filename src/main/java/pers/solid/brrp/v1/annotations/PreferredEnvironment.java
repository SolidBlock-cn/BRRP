 ...
 * <p>Remember that <em>this annotation is only a notice. It does not take real effect.</em></p>
 *
...
public @interface PreferredEnvironment {
  /**
   * Represents the environment that the annotated element is preferred to be only present in, and overriding methods are preferred to be annotated with <code>@Environment(<var>the value</var>)</code>.
   */
  EnvType value();
}
