...
public interface RecipeJsonBuilderExtension<Self> {
  ...
  /**
   * Add a criterion of obtaining the item ({@link RecipesProvider#conditionsFromItem(ItemConvertible)}), with the auto-decided criterion name ({@link RecipesProvider#hasItem(ItemConvertible)}. This is a convenient method.
   */
  default Self criterionFromItem(ItemConvertible item) {
    return criterionMethodBridge(RecipesProvider.hasItem(item), RecipesProvider.conditionsFromItem(item));
  }
  ...
  /**
   * Add a criterion of obtaining the item ({@link RecipesProvider#conditionsFromItem(NumberRange.IntRange, ItemConvertible)} with the specified criterion name.
   */
  default Self criterionFromItem(String criterionName, NumberRange.IntRange count, ItemConvertible item) {
    return criterionMethodBridge(criterionName, RecipesProvider.conditionsFromItem(count, item));
  }
