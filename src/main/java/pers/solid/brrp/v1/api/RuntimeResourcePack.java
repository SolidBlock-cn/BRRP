...
public interface RuntimeResourcePack extends ResourcePack {
  ...
  Gson GSON = LootGsons.getTableGsonBuilder()
      ...
      .registerTypeHierarchyAdapter(Vec3f.class, JsonSerializers.VECTOR_3F)
      ...
      .setPrettyPrinting()
      .create();
  ResourcePackSource RUNTIME = ResourcePackSource.method_29486("pack.source.runtime");
   ...
   * @param lootTable  The loot table object. It can be created through {@link LootTable#builder()}. Please also refer to {@link LootTablesProvider} for some methods to conveniently create loot tables.
   */
  @Contract(mutates = "this")
   ...
   *
   * @param identifier The identifier of the loot table. Please refer to {@link AbstractBlock#getLootTableId()}.
   * @param lootTable  The loot table (builder) object. It can be created through {@link LootTable#builder()}. Please also refer to {@link LootTablesProvider} for some methods to conveniently create loot tables.
   */
  @Contract(mutates = "this")
   ...
   * @param state The {@link BlockStateSupplier} file. You can also easily create them using methods in {@link net.minecraft.data.client.model.BlockStateModelGenerator}.
   */
  @Contract(mutates = "this")
   ...
   * @param tagKey     The vanilla {@link Tag.Identified} object. It is usually created through APIs of the mod loader (Fabric, Forge, etc). You can find vanilla ones in classes like {@link BlockTags} and {@link ItemTags}.
   * @param tagBuilder The vanilla {@link Tag.Builder} object. You may also find {@link ObjectTagBuilder} and {@link IdentifiedTagBuilder} provided by this mod.
   * @param outputPath The name of the tag output path, not including the namespace and the prefix "{@code tags/}". Some examples: {@code blocks}, {@code items}.
   */
  @Contract(mutates = "this")
  <T> byte[] addTag(Tag.Identified<T> tagKey, Tag.Builder tagBuilder, String outputPath);
  ...
  default <T> byte[] addTag(IdentifiedTagBuilder<T> identifiedTagBuilder) {
    return addTag(identifiedTagBuilder.tag, identifiedTagBuilder, identifiedTagBuilder.dataType);
  }
   ...
   * @param recipe The {@link RecipeJsonProvider} object. You may conveniently create it using methods in {@link RecipesProvider}.
   */
  @Contract(mutates = "this")
  ...
  /**
   * Add a recipe <em>as well as</em> a corresponding advancement to obtain that recipe. <strong>The {@link RecipeJsonFactory} must have an {@link ItemGroup}, or it will throw an exception.</strong>
   *
   * @param recipeId          The id of the recipe.
   * @param recipeJsonBuilder The {@link RecipeJsonFactory}. The id of the advancement will be determined by {@link RecipeJsonFactory#offerTo}.
   */
  @Contract(mutates = "this")
  default void addRecipeAndAdvancement(Identifier recipeId, @NotNull RecipeJsonFactory recipeJsonBuilder) {
   ...
   *
   * @param id    The id of the model. For block models, it is referred to in the {@link net.minecraft.data.client.model.BlockStateModelGenerator}. Some examples: {@code new Identifier("my_mod", "block/my_stone_slab_top")}, {@code new Identifier("my_mod", "item/my_item")}.
   ...
   */
  @Contract(mutates = "this")
  default byte[] addAdvancement(Identifier id, Advancement.Task advancement) {
  ...
  default Text getDisplayName() {
    return new TranslatableText("brrp.pack.defaultName", getId());
  }
