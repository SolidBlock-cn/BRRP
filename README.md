...
以测试，上述三种实现方式的**性能差异不大**。这也就意味着，对目前版本而言，运行时资源包并没有起到性能提升的效果。而模组本身因涉及复杂的修改，难免存在兼容性和维度难度的问题。因此作者的其他模组，未来将不再依赖此模组。实际上，Fabric API 自带的 Fabric Data Generation API 兼容性更好，且也在更新与维护，不需要消耗我自己的精力，所以更值得使用。
